...
# Applications
yay -S librewolf-bin anki-bin obs-studio thunar thunar-archive-plugin xarchiver unzip # vscodium-bin zathura zathura-pdf-mupdf  imv mpv gimp flameshot imagemagick calibre gvfs gvfs-mtp
	
# CLI
yay -S bottom hledger-bin # tldr pass pass-otp fzf ripgrep  slidev-bin libqalculate bat duf gdu navi jump hugo dust cfdisk tokei pdftk qpdf csview gparted testdisk neofetch visidata czkawka-cli nvtop hyperfine stress-ng 7zip qrcp feh xclip cronie man-db gpg-tui mediainfo perl-image-exiftool scrcpy newsboat slides-bin lf-bin yazi ncmpcpp mpc mpd pulsemixer pandoc-bin beamer-bin texlive-core calcurse
